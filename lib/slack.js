...
function createStatusObject(inData, userProfile) {
  ...
  if (inData.quittingTime !== undefined &&
    ...
    let startAndQuit = setUpStartAndQuitTime(inData, inData.eventEndTime);
    console.log("entimeasdat:", inData.eventEndTimeAsDate);
    console.log("quittingtime", startAndQuit.quitting);
    if ((inData.eventEndTimeAsDate >= startAndQuit.quittingTime) ||
      (inData.eventEndTimeAsDate <= startAndQuit.startingTime)) {
      statusObject.statusText = inData.workdayOverStatus;
      statusObject.statusEmoji = inData.notAtWorkEmoji;
      console.log("Returning work day over", statusObject);
      return statusObject;
    }
  ...
  if (inData.eventBeginTime !== undefined &&
    ...
    let startAndQuit = setUpStartAndQuitTime(inData, inData.eventBeginTime);
    console.log(">>beginTimeAsDate:", inData.eventBeginTimeAsDate, "startingTime", startAndQuit.startingTime);
    if (inData.eventBeginTimeAsDate <= startAndQuit.startingTime) {
...
function setUpStartAndQuitTime(inData, dateAsString) {
  let timezoneOffset = getTimeZoneOffset(dateAsString),
    quitTime = new Date(Date.now()),
    startTime = new Date(Date.now()),
    quitTimeParts = inData.quittingTime.split(":"),
    startTimeParts = inData.startingTime.split(":"),
    offsetParts = timezoneOffset.split(":");

  let quitHours = (parseInt(quitTimeParts[0]) + parseInt(offsetParts[0])) % 24,
    quitMinutes = (parseInt(quitTimeParts[1]) + parseInt(offsetParts[1])) % 60,
    startHours = (parseInt(startTimeParts[0]) + parseInt(offsetParts[0])) % 24,
    startMinutes = (parseInt(startTimeParts[1]) + parseInt(offsetParts[1])) % 60;

  quitTime.setHours(quitHours, quitMinutes, 0, 0);
...
function getTimeZoneOffset(dateAsString) {
  //The dates get passed from the Google calendar app with the timeoffset
  //at the end of the string so after the first two hypens after year and month
  //use 10 as a safe starting index
  let safeStartIndex = 10,
    offsetIdx = dateAsString.indexOf('-', safeStartIndex);
  return dateAsString.substring(offsetIdx);
}
...
module.exports = {
  ...
  _setupStartAndQuitTime: setUpStartAndQuitTime
};